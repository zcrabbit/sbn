...
class SBN:
    ...
    def check_clade_dict_em(self):
        ...
        for key in self.clade_double_bipart_dict:
            ...
            if parent_clade_bitarr.count() != self.ntaxa:
                print '{}|{}:{:.12f}|{:.12f}'.format(parent_clade_bitarr.to01(), bipart_bitarr.to01(), sum(self.clade_double_bipart_dict[key].values()), self.clade_bipart_dict[parent_clade_bitarr.to01()][bipart_bitarr.to01()])
            else:
    ...
    def bn_em_count(self, tree_count, tree_id, maxiter=100, miniter=50, abstol=1e-04, monitor=False, MAP=False):
        ...
        for k in range(maxiter):
            ...
            for key in tree_count:
                ...
                est_prob = self.bn_dict_em_update(tree, count/total_count, bipart_bitarr_prob, clade_dict, clade_bipart_dict, clade_double_bipart_dict)                
    ...
    def set_clade_bipart(self, clade_dict, clade_bipart_dict, clade_double_bipart_dict):
        self.clade_dict, self.clade_bipart_dict, self.clade_double_bipart_dict = deepcopy(clade_dict), deepcopy(clade_bipart_dict), deepcopy(clade_double_bipart_dict)
    ...
    def ccd_estimate(self, tree, unrooted=True):
        ...
        for node in tree.traverse('levelorder'):
            if node.is_root():
                ...
                ccd_est *= self.clade_bipart_dict[nodetobitMap[node].to01()][bipart_bitarr.to01()]/self.clade_dict[self._minor_bitarr(nodetobitMap[node]).to01()]
    ...
    def bn_estimate_rooted(self, tree, MAP=False):
        ...
        for node in tree.traverse('levelorder'):
            if node.is_root():
                ...
                bn_est *= (self.clade_dict[self._minor_bitarr(child_bitarr).to01()]+MAP*self.alpha*normalizing_const_est)/(1.0+MAP*self.alpha)
            elif not node.is_leaf():
                for child in node.children:
                    child_bitarr = self.clade_to_bitarr(child.get_leaf_names())
                    nodetobitMap[child] = child_bitarr
                ...
                if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()] > 0:
                    bn_est *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                else:
                    bn_est = 0.0; break
    ...
    def _bn_estimate_fast(self, tree, MAP=False):
        ...
        for node in tree.traverse('postorder'):
            if not node.is_leaf() and not node.is_root():
                ...
                if not node.up.is_root():
                    ...
                    normalizing_const = self.clade_bipart_dict[nodetobitMap[node.up].to01()][parent_bipart_bitarr.to01()]
                    normalizing_const_est = self.clade_bipart_freq_est[nodetobitMap[node.up].to01()][parent_bipart_bitarr.to01()]
                    if (normalizing_const + MAP*self.alpha*normalizing_const_est) == 0.0 or self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()] == 0.0:
                        cbn_est_up[node] = 0.0
                    else:
                        cbn_est_up[node] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()] + MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const + MAP*self.alpha*normalizing_const_est)
        ...
        for node in tree.traverse('preorder'):
            if not node.is_root():
                if node.up.is_root():
                        ...
                        for sister in node.get_sisters():
                            if not sister.is_leaf():
                                ...
                                if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()] > 0.0:
                                    cbn_est_down[node] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr.to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                                else:
                                    cbn_est_down[node] = 0.0
                else:
                    ...
                    if (normalizing_const+MAP*self.alpha*normalizing_const_est) == 0.0:
                        ...
                        if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_down[node.up].to01()] > 0.0:
                            cbn_est_down[node] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_down[node.up].to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                        else:
                            cbn_est_down[node] = 0.0
                        ...
                        if not sister.is_leaf():
                            ...
                            if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_up[sister].to01()] > 0.0:
                                cbn_est_down[node] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_up[sister].to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                            else:
                                cbn_est_down[node] = 0.0
                ...
                bipart_bitarr_prob[parent_bipart_bitarr.to01()] = (self.clade_dict[parent_bipart_bitarr.to01()]+MAP*self.alpha*normalizing_const_est)/(1.0+MAP*self.alpha)
                ...
                if (normalizing_const+MAP*self.alpha*normalizing_const_est) == 0.0:
                    ...
                    if not node.is_leaf():
                        ...
                        if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_up[node].to01()] > 0.0:
                            bipart_bitarr_prob[parent_bipart_bitarr.to01()] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_up[node].to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                        else:
                            bipart_bitarr_prob[parent_bipart_bitarr.to01()] = 0.0
                    ...
                    if self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_down[node].to01()] > 0.0:
                        bipart_bitarr_prob[parent_bipart_bitarr.to01()] *= (self.clade_double_bipart_dict[comb_parent_bipart_bitarr.to01()][bipart_bitarr_down[node].to01()]+MAP*self.alpha*normalizing_const_est/self.clade_double_bipart_len[comb_parent_bipart_bitarr.to01()])/(normalizing_const+MAP*self.alpha*normalizing_const_est)
                    else:
                        bipart_bitarr_prob[parent_bipart_bitarr.to01()] = 0.0
